...
from appmodels import Role, User, UserRole
...
class UserService(SQLAlchemyAsyncRepositoryService[User]):
    ...
    async def create(
        ...
        *,
        ...
        '''Create a new User and assign the deafult role.'''
        if isinstance(data, dict):
            role_id: UUID | None = data.pop("role_id", None)
            data = await self.to_model(data, "create")
            if role_id:
                data.roles.append(UserRole(role_id=role_id, assigned_at=datetime.now(timezone.utc)))
